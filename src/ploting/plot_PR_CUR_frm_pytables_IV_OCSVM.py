...
def plot_data(Res, kfolds, featr_size_lst, genres, nu_lst):
      
    color = ['c', 'c', 'c', 'k', 'k', 'k', 'k', 'k', 'r', 'r', 'r', 'r', 'r', 'r', 'r', 'r', 'r', 'r'  ]
    symbol = [ "*", "^", "x", "+", "^" , "*", "+", "x", "*", "^", "x", "+", "^" , "*", "+", "x" ]
    line_type = [ "-", "-", "-", "-", "--" , "--", "--", "--", "-", "-", "-", "-", "--" , "--", "--", "--" ]
    ...
    for i_fs, featr_size in enumerate(featr_size_lst):
        ...
        for i_nu, nu in enumerate(nu_lst):
            ...
            plt.xlabel( 'R' )
            plt.ylabel( 'P' ) 
            plt.plot(R, P, color[i_nu] + symbol[i_nu] + line_type[i_nu], label=str(featr_size)+" - "+str(nu))
            plt.grid(True)    
            plt.legend(loc=3 )
...
if __name__ == '__main__':
    ...
    featr_size_lst = [70000] #1000, 5000, 10000,
    gnr_num = 7
    ...
    plot_data(CrossVal_Kopples_method_res, kfolds, featr_size_lst, genres, nu_lst)
