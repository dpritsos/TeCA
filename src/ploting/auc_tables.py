...
def h5d_auc_table(h5d_fl1, h5d_fl2, kfolds, params_od, mix, is_ttbl, strata):
    ...
    for params_lst, params_path in zip(
        ...
        if params_lst[0] > params_lst[1]:
            ...
            #Extending paramters list with AUC(s).
            params_lst.extend(auc_values)

            #Appending the paramters list toghether with their respective AUC(s).
            res_lst.append(params_lst)

    #Stacking and returning the data collected in a 2D array. Last column contain the AUC for every
    #parameters values possible combination.
