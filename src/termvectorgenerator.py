...
class BaseHTMLFileTools(object):
    ...
    def __load_html_file(self, filename, encoding='utf8', error_handling='strict'):
        ...
        try:
            fenc = codecs.open( filename, 'r', encoding, error_handling)
        except IOError as e:
    ...
    def load_html_files(self, filename_l, encoding='utf8', error_handling='strict'):
        ...
        if isinstance(filename_l, str):
            return [ self.load_html_file(filename_l, encoding, error_handling) ]
...
class BaseRegexTools(object):
    ...
    def extract_text(self, xhtml_str):
        ...
        if not properhtml:
            ...
            try:
                unicod_text = unicodedata.normalize('NFKC', text.decode())
            except:
                unicod_text = unicodedata.normalize('NFKC', text)
        return unicod_text
...
class HTML2NgFTools(BaseHTMLFileTools):
    ...
    def nf_d_frmpaths(self, basepath, filepath_l, encoding='utf8', error_handling='strict'):
...
class HTML2TFTools(BaseHTML2TFRegexTools, BaseRegexTools, BaseHTMLFileTools):
    ...
    def tf_d_frmpaths(self, basepath, filepath_l, encoding='utf8', error_handling='strict'):
        wbpg_l, html_str_l = self.load_html_frmpaths(basepath, filepath_l, encoding, error_handling)
...
if __name__ == "__main__":
    ...
    wpg_l, tf_d_l = html2nf.nf_d_frmpaths(None, files_path, error_handling='ignore')
    print wpg_l[0] 
    print tf_d_l[0]

    wpg_l, tf_d_l = html2tf.tf_d_frmpaths(None, files_path, error_handling='ignore')
    print wpg_l[0] 
