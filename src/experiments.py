...
import copy
from stemmers import MyStemmer 
import math
...
class SVMExperiments(object):
    ...
    def load_tt_sets(self, base, tt_path, tt_pg_lim, genres, lower_case):
        ...
        tt_tf_d_l = list()
        for rst_g in genres:
            wpgl, tfdl = VHTools.load_tfd_l_frmpaths(None, [ base + rst_g + tt_path ], page_lim=tt_pg_lim, force_lower_case=lower_case) #base + rst_g + test_path
            ## Maybe the following shorting patch should be transfered to the Save function in vectohandlingtools module
            mrgd_l = list()
    ...
    def DEPRICATED_tf_experiment_set1(self, base_filepath, training_path, train_pg_lim, test_path, test_pg_lim, g, genres, lower_case, fmethod="w", keep_terms=None):
        #Load TF Dictionary
        for base in base_filepath:
            filepath = base + g
            dict_filepath = base + g + training_path    
        ...
        #Stemming the terms the training set using Porter's Stemmer
        ##train_tf_d_l = self.porter.stem_dl(train_tf_d_l)
        #if train_tf_d_l_new == train_tf_d_l:
        ...
        del wpg, tfdl
        #Stemming the terms the test set using Porter's Stemmer
        ##test_tf_d_l = self.porter.stem_dl(test_tf_d_l)
        ...
        for i in [2,3]:
            ...
            if i == 1:
                ...
                train_idxf_d_l_frmd = vformat.tf2tf_idf(copy.deepcopy(train_idxf_d_l), idxf_d)
                test_idxf_d_l_frmd = vformat.tf2tf_idf(copy.deepcopy(test_idxf_d_l), idxf_d)
            for nu in [0.05, 0.07, 0.1, 0.15, 0.2, 0.3, 0.5, 0.7, 0.8]: #[0.2, 0.3, 0.5, 0.7, 0.8]: # 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9]:
                ############################################## Train SVM ###############################################
                fobj.write("++++ for nu= " + str(nu) + " ++++\n")
                print "Training"
                #for size in range(1500,9000,500):
    ...
    def DEPRICATED_tf_experiment_set2(self, base_filepath, training_path, train_pg_lim, test_path, test_pg_lim, genres, lower_case):
        train_tf_d_l = list()
        test_tf_d_l = list()
        for g in genres:
            #Load TF Dictionary
    ...
    def DEPRICATED_tf_experiment_set3(self, keep_term_lims, base_filepath, training_path, train_pg_lim, test_path, test_pg_lim, g, genres, lower_case):
